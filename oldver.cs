...
namespace Blinky
{
    public class Program
    {
        ...
        {
            foreach pair in PinButtonPair{
                pair.Button.read() = pair.pin.PinValue.toggle();
                if pressedsequence != sequence
                    play sequenece again
                if pressed sequence == sequence

               convert timer to score
            <0
        }
